...
class LQG:
    ...
    def update_lqr_analytic(self, goal_state, infinite_horizon=False):
        ...
        # get matrix K
        K = np.zeros((num_rows, num_cols))  # shape (p^3, p)
        for i in range(num_cols):
            e = np.zeros((num_cols, 1)) # shape (p, 1)
            e[i] = 1
            col = Vec(np.kron(e, I_p))  # shape (p^3, 1)
            K[:, i] = col

        # get matrix Z, Z expected to be shape (p^3, 1)
        Z1 = np.kron(A, B) @ np.kron(I_p, x) # shape (n^2, p)
       
        u_temp = -1/2 * np.linalg.pinv(K.T @ K) @ K.T  # shape (p, p^3)
